    ...
    try {
      for (let start = 0; start < evaluated.length; start += BATCH_SIZE) {
        ...
        for (const row of batch) {
          console.log('[inventory-import]', {
            categoryName: row.normalized.categoryName,
            subcategoryName: row.normalized.subcategoryName,
            name: row.normalized.rawName,
          });
        }
  ...
  private async resolveImportedCategory(
    ...
    if (!subcategory) {
      throw new NotFoundException(
        `Подкатегория «${subcategoryName}» не найдена ` +
