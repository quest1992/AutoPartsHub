  ...
  it('loads a complete brand → model → generation → engine tree', async () => {
    const tree = [{
      ...
      vehicleModels: [{
        id: 'model',
        name: 'Camry',
        generations: [{ id: 'generation', name: 'XV70', engines: [{ id: 'engine', code: 'A25A' }] }],
      }],
    }];
    const prisma = { manufacturer: { findMany: jest.fn().mockResolvedValue(tree) } };
    ...
    expect(result[0].vehicleModels[0].generations[0].engines[0].code).toBe('A25A');
