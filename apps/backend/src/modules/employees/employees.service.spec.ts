...
const sensitive = (value: unknown) =>
  expect(JSON.stringify(value)).not.toMatch(/passwordHash|temporaryPassword/);
...
describe('EmployeesService', () => {
  ...
  it('enforces shop context for SHOP_ADMIN and SUPER_ADMIN', async () => {
    await expect(
      service.list(actor, { shopId: 'other' }),
    ).rejects.toBeInstanceOf(ForbiddenException);
    await expect(
      service.list({ ...actor, shopId: null }, {}),
    ).rejects.toBeInstanceOf(ForbiddenException);
    await expect(service.list(superActor, {})).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  it('excludes SUPER_ADMIN and paginates safely', async () => {
    prisma.user.findMany.mockResolvedValue([employee]);
  ...
  it('creates an employee with bcrypt hash and no sensitive response', async () => {
    prisma.user.findUnique.mockResolvedValue(null);
    prisma.user.create.mockResolvedValue(employee);
  ...
  it('hides foreign employees and blocks self actions', async () => {
    prisma.user.findFirst.mockResolvedValue(null);
    await expect(service.findOne(actor, 'x')).rejects.toBeInstanceOf(
      NotFoundException,
    );
    ...
    await expect(
      service.update(actor, actor.id, { isActive: false }),
    ).rejects.toBeInstanceOf(ForbiddenException);
  });
  it('protects last SHOP_ADMIN with Serializable transaction', async () => {
    const target = { ...employee, role: UserRole.SHOP_ADMIN };
    tx.user.findFirst.mockResolvedValue(target);
    tx.user.count.mockResolvedValue(0);
    await expect(
      service.update(superActor, target.id, { isActive: false }, 'shop-1'),
  ...
  it('resets password without exposing hash', async () => {
    prisma.user.findFirst.mockResolvedValue(employee);
    prisma.user.update.mockResolvedValue({});
    const result = await service.resetPassword(actor, employee.id, {
      temporaryPassword: 'Password2',
    });
    expect(hash).toHaveBeenCalledWith('Password2', 12);
