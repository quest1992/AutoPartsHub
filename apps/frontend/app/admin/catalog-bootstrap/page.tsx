...
export default function CatalogBootstrapPage() {
  ...
  async function autoCreateSafe(){
    if(!window.confirm('Автоматически создать до 200 позиций только по подтверждённым безопасным правилам?'))return;
    setCreating(true);setError('');
    try{
  ...
  return <ProtectedLayout>
    ...
    <section>
      ...
      {autoReport&&<p className="mt-4 rounded bg-emerald-50 p-3 text-emerald-900">Автозаполнение: найдено категорий {autoReport.categoriesFound}, безопасных {autoReport.safeRecognized}, создано {autoReport.created}, уже существовало {autoReport.skippedExisting}, небезопасных {autoReport.skippedUnsafe}, ошибок {autoReport.failed}, отложено {autoReport.deferred}.</p>}
